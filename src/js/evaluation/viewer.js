...
export function createEvaluationViewer(root) {
  ...
  const api = { load(summary, decisions = null) { state.run = normalizeEvaluationRun(summary, decisions); state.gameIndex = 0; state.decisionIndex = 0; render(); return state.run; },
    loadCounterfactuals(raw) { const normalized = normalizeCounterfactualCollection(raw); state.counterfactuals = normalized.analyses; state.counterfactualSampling = normalized.sampling || null; state.counterfactualDiagnostics = normalized.diagnostics || null; render(); }, state };
  const analysisFor = event => state.counterfactuals.find(item => item.matchId === event.matchId && item.decisionIndex === event.decisionIndex);
  ...
  function filteredDecisions(game) {
    return (game?.decisionEvents || []).filter(event => {
      ...
      const analysis = analysisFor(event);
      if (state.filter === 'face-vs-trade' && !(analysis?.disagreement?.jevChoiceClass === 'attack-face' && analysis?.disagreement?.neuralChoiceClass === 'attack-minion')) return false;
      if (state.filter === 'face-vs-board' && !(analysis?.disagreement?.jevStrategic === 'face-pressure' && analysis?.disagreement?.neuralStrategic === 'board-control')) return false;
      if (state.filter === 'jev-end' && analysis?.selectedActionType !== 'end-turn') return false;
      if (state.filter === 'jev-power' && analysis?.selectedActionType !== 'hero-power') return false;
      if (state.filter === 'missed-lethal' && !(analysis?.lethal?.lethalAvailable
        && (!analysis.lethal.jevSelectedLethal || !analysis.lethal.neuralSelectedLethal))) return false;
  ...
  function render() {
    ...
    if (state.counterfactuals.length) { const cf = counterfactualSummary(state.counterfactuals, state.analysisDifferenceThreshold); const title = el('h3', '', 'Deep MCTS counterfactual estimates'); summary.append(title); const cfMetrics = el('div', 'summary-grid'); cfMetrics.append(metric('Positions analyzed', cf.positionsAnalyzed), metric('Jev-selected estimated higher', cf.jevHigher), metric('Neural-selected estimated higher', cf.neuralHigher), metric('Within threshold', cf.approximatelyTied), metric('Mean value difference', number(cf.meanDifference)), metric('Median value difference', number(cf.medianDifference))); const sampling = state.counterfactualSampling; if (sampling) cfMetrics.append(metric('Sampling strategy', sampling.strategy), metric('Eligible positions', sampling.eligiblePositions), metric('Matches represented', `${sampling.sampledMatches} / ${sampling.eligibleMatches}`)); summary.append(cfMetrics, el('p', 'muted', `Descriptive results from the configured evaluator; close threshold ±${cf.threshold}.`)); if (sampling?.actionTypes) { const types = el('div', 'config'); for (const [type, count] of Object.entries(sampling.actionTypes)) types.append(el('span', 'chip', `sample ${type}: ${count}`)); summary.append(types); } const diagnostics = state.counterfactualDiagnostics; if (diagnostics?.tacticalPairs?.length) { const tactical = el('div', 'config'); diagnostics.tacticalPairs.slice(0, 8).forEach(pair => tactical.append(el('span', 'chip', `${pair.jevChoice} vs ${pair.neuralChoice}: ${pair.count}, median ${number(pair.medianValueDifference)}`))); summary.append(el('h3', '', 'Tactical pairs'), tactical); } }
    ...
    const controls = el('div', 'controls'); const filter = document.createElement('select'); filter.innerHTML = '<option value="all">All decisions</option><option value="disagreements">Disagreements only</option><option value="agreements">Agreements only</option><option value="jev">Jev decisions only</option><option value="jev-mcts">Jev ≠ MCTS</option><option value="attack">Attacks</option><option value="play-card">Card plays</option><option value="face-vs-trade">Jev face vs Neural trade</option><option value="face-vs-board">Jev pressure vs Neural control</option><option value="jev-end">Jev end turn</option><option value="jev-power">Jev hero power</option><option value="missed-lethal">Missed lethal</option>'; filter.value = state.filter; filter.onchange = () => { state.filter = filter.value; render(); }; const search = document.createElement('input'); search.type = 'search'; search.placeholder = 'Search cards, actions, agents…'; search.value = state.search; search.onchange = () => { state.search = search.value; render(); }; controls.append(filter, search); timelinePanel.append(controls);
  ...
  function renderDecision(game, event) {
    ...
    const analysis = analysisFor(event);
    if (analysis) { const panel = el('div', 'panel'); panel.append(el('h3', '', 'Deep MCTS analysis')); if (analysis.status === 'error') panel.append(el('div', 'error-banner', analysis.message || analysis.errorType)); else { const jev = analysis.candidates?.find(candidate => candidate.selectedBy?.includes('jev')); const neural = analysis.candidates?.find(candidate => candidate.selectedBy?.includes('neural')); for (const candidate of analysis.candidates || []) { const effects = candidate.immediateEffects || {}; panel.append(metric(`${candidate.selectedBy.join(' + ') || 'Candidate'} · ${candidate.classification?.primary || 'unclassified'} · ${candidate.description}`, `${candidate.estimatedValue >= 0 ? '+' : ''}${number(candidate.estimatedValue)}${candidate.runs?.length > 1 ? ` ± ${number(candidate.stdDev)}` : ''}`), el('p', 'muted', `Immediate face damage ${effects.opponentHeroDamageImmediate ?? '—'} · enemy minions removed ${effects.enemyMinionsRemoved ?? '—'} · enemy board attack removed ${effects.enemyBoardAttackDelta ?? '—'} · lethal ${candidate.immediateLethal ? 'yes' : 'no'}`)); } if (jev && neural) { const difference = jev.estimatedValue - neural.estimatedValue; const label = difference > state.analysisDifferenceThreshold ? 'Jev-selected action estimated higher' : difference < -state.analysisDifferenceThreshold ? 'Neural-selected action estimated higher' : 'Values very close'; panel.append(metric(label, `${difference >= 0 ? '+' : ''}${number(difference)}`)); } if (analysis.sequencing) panel.append(el('p', 'muted', `Remaining mana ${analysis.sequencing.remainingMana} · non-end actions ${analysis.sequencing.nonEndLegalActionCount} · playable card ${analysis.sequencing.hadPlayableCard ? 'yes' : 'no'} · attack ${analysis.sequencing.hadAvailableAttack ? 'yes' : 'no'} · hero power ${analysis.sequencing.hadHeroPowerAvailable ? 'yes' : 'no'}`)); const cfg = analysis.evaluator || {}; panel.append(el('p', 'muted', `Iterations ${cfg.iterations} · depth ${cfg.rolloutDepth} · repeats ${cfg.repeats} · ${cfg.fullSim ? 'full simulation' : 'lightweight simulation'} · policy guidance ${cfg.policyGuidance} · seed ${cfg.baseSeed} · information mode ${cfg.informationMode}. Jev input mode: player-visible.`)); } wrap.append(panel); }
